 ...
 */
export async function clearPropertiesCache(purpose) {
  if (purpose) {
...
export async function getPropertiesByPurpose(purpose) {
  try {
    // Use only Next.js fetch cache for listing pages
    // In-memory cache is not reliable with serverless instances
    // and prevents proper revalidation across instances

    // Level 1: Next.js fetch cache (persistent & CDN-backed)
    const endpoint = PROPERTIES_BY_PURPOSE_ENDPOINT(purpose);
    const cacheBuster = `?_cache=${Date.now()}`; // Force fresh API response
    console.log(`📡 Fetching properties from: ${endpoint}${cacheBuster}`);
...
export async function getPropertyBySlug(slug) {
  try {
    ...
    const cacheBuster = `?_cache=${Date.now()}`; // Force fresh API response
    console.log(`📡 Fetching property from: ${endpoint}${cacheBuster}`);
