...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: Montiva by Vida`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: Montiva by Vida`);
    ...
    const sanitizedData = {
      ...
      unitType: data.unitType ? escapeHtml(data.unitType.trim()) : "",
      message: data.message ? escapeHtml(data.message.trim()) : "",
      createdAt: data.createdAt || new Date().toISOString(),
      source: data.source ? escapeHtml(data.source.trim()) : "Montiva by Vida - Dubai Creek Harbour",
    };
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .value { color: #333; }
            .footer { background: #059669; color: white; padding: 15px; text-align: center; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="header">
            <h2>Montiva by Vida - New Inquiry</h2>
          ...
          <div class="content">
            ...
            ${
              sanitizedData.unitType
                ? `
            <div class="data-item">
              <span class="label">Unit Type:</span>
              <span class="value">${sanitizedData.unitType}</span>
            </div>
            `
                : ""
            }
          ...
          <div class="footer">
            <p>Montiva by Vida Landing Page Submission</p>
          </div> 
        </body>
      </html>
    `;
    ...
    // Plain text version
    const emailText = `
MONTIVA BY VIDA - NEW INQUIRY
==============================

Name: ${sanitizedData.fullName}
Phone: ${sanitizedData.phoneNumber}
Email: ${sanitizedData.email}
${sanitizedData.unitType ? `Unit Type: ${sanitizedData.unitType}\n` : ""}${
...
Montiva by Vida Landing Page
    ...
    // Send Email
    const mailOptions = {
      from: `Montiva by Vida <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      replyTo: sanitizedData.email,
      subject: `Montiva by Vida Inquiry: ${sanitizedData.fullName}`,
      text: emailText,
      html: emailHtml,
    };
    ...
    try {
      ...
      console.log(`✅ Valid submission from IP: ${ip} | Email: ${sanitizedData.email.split("@")[0]}@*** | Form: Montiva by Vida`);
      ...
      console.error(`❌ Error from IP: ${ip} | Form: Montiva by Vida`, emailError);
      return NextResponse.json(
        {
          success: false,
          message:
    ...
    console.error(`❌ Error from IP: ${ip} | Form: Montiva by Vida`, error);
    return NextResponse.json(
      {
        success: false,
        message:
