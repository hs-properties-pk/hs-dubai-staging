...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: Eltiera Views`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: Eltiera Views`);
    ...
    const sanitizedData = {
      ...
      apartmentType: data.apartmentType ? escapeHtml(data.apartmentType.trim()) : "",
      message: data.message ? escapeHtml(data.message.trim()) : "",
      createdAt: data.createdAt || new Date().toISOString(),
      source: data.source ? escapeHtml(data.source.trim()) : "Eltiera Views - Jumeirah Islands",
    };
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .data-item { margin-bottom: 15px; padding: 10px; background: white; border-left: 4px solid #0284c7; }
            .label { font-weight: bold; color: #0284c7; }
            .value { color: #1e293b; }
            .footer { background: #0369a1; color: white; padding: 15px; text-align: center; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="header">
            <h2>Eltiera Views - New Inquiry</h2>
          ...
          <div class="content">
            ...
            ${
              sanitizedData.apartmentType
                ? `
            <div class="data-item">
              <span class="label">Apartment Type:</span>
              <span class="value">${sanitizedData.apartmentType}</span>
            </div>
            `
                : ""
            }
          ...
          <div class="footer">
            <p>Eltiera Views Landing Page Submission</p>
          </div> 
        </body>
      </html>
    `;
    ...
    // Plain text version
    const emailText = `
ELTIERA VIEWS - NEW INQUIRY
============================

Name: ${sanitizedData.fullName}
Phone: ${sanitizedData.phoneNumber}
Email: ${sanitizedData.email}
${sanitizedData.apartmentType ? `Apartment Type: ${sanitizedData.apartmentType}\n` : ""}${
...
Eltiera Views Landing Page
    ...
    // Send Email
    const mailOptions = {
      from: `Eltiera Views <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      replyTo: sanitizedData.email,
      subject: `Eltiera Views Inquiry: ${sanitizedData.fullName}`,
      text: emailText,
      html: emailHtml,
    };
    ...
    try {
      ...
      console.log(`✅ Valid submission from IP: ${ip} | Email: ${sanitizedData.email.split("@")[0]}@*** | Form: Eltiera Views`);
      ...
      console.error(`❌ Error from IP: ${ip} | Form: Eltiera Views`, emailError);
      return NextResponse.json(
        {
          success: false,
          message:
    ...
    console.error(`❌ Error from IP: ${ip} | Form: Eltiera Views`, error);
    return NextResponse.json(
      {
        success: false,
        message:
