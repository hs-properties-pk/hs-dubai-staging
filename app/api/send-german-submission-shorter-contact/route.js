...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: German Shorter Contact`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
    console.log(`🚫 Bot detected from IP: ${ip} | Form: German Shorter Contact`);
    ...
    <table style="border-collapse: collapse; width: 100%;">
      ...
      <tbody>
        <tr><td style="border: 1px solid #ddd; padding: 8px;">Name</td><td style="border: 1px solid #ddd; padding: 8px;">${data.name}</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;">Email</td><td style="border: 1px solid #ddd; padding: 8px;">${data.email}</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;">Phone Number</td><td style="border: 1px solid #ddd; padding: 8px;">${data.phoneNumber}</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;">Bedroom Count</td><td style="border: 1px solid #ddd; padding: 8px;">${data.bedroomCount}</td></tr>
        <tr><td style="border: 1px solid #ddd; padding: 8px;">Submitted At</td><td style="border: 1px solid #ddd; padding: 8px;">${data.createdAt}</td></tr>
      </tbody>
    </table>
    `;
    ...
    sendLeadEvent({
      ...
      eventSource: data.source || "German Shorter Contact Form",
    ...
    const { sheetSaved, emailSent, thankYouSent, notionSaved } = await runParallelLeadDispatch({
      ...
      leadNotification: trySendIpagMail({
        ...
        text: `German shorter form — ${data.name}`,
        html: emailHtml,
      }),
      ...
      notion: appendLeadToNotion({
        ...
        enquiryType: "German Shorter Contact",
        page: data.source || "German Shorter Contact Form",
        date: createdAt,
        extra: data.bedroomCount != null && String(data.bedroomCount).trim() !== ""
    ...
    console.log(
      `✅ German shorter IP: ${ip} | ${data.email.split("@")[0]}@*** | sheet=${sheetSaved} email=${emailSent} thankYou=${thankYouSent} notion=${notionSaved}`
    );
    ...
    console.error(`❌ Error from IP: ${ip} | Form: German Shorter Contact`, error);
    return NextResponse.json(
