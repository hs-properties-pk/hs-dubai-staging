...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: Dubai Creek Harbour`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: Dubai Creek Harbour`);
    ...
    const sanitizedData = {
      ...
      source: data.source ? escapeHtml(data.source.trim()) : "Dubai Creek Harbour",
    };
    ...
    const emailHtml = `
      ...
      <html>
        ...
        <body>
          <div class="header">
            <h2>Dubai Creek Harbour - New Inquiry</h2>
          ...
          <div class="footer">
            <p>Dubai Creek Harbour Landing Page Submission</p>
          </div> 
        </body>
      </html>
    `;
    ...
    // Plain text version
    const emailText = `
Dubai Creek Harbour - NEW INQUIRY
========================================
...
========================================
Dubai Creek Harbour Landing Page
    `;
    ...
    // Send Email
    const mailOptions = {
      from: `Dubai Creek Harbour <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      replyTo: sanitizedData.email,
      subject: `Dubai Creek Harbour Inquiry: ${sanitizedData.fullName}`,
      text: emailText,
      html: emailHtml,
    };
    ...
    try {
      ...
      console.log(`✅ Valid submission from IP: ${ip} | Email: ${sanitizedData.email.split("@")[0]}@*** | Form: Dubai Creek Harbour`);
    ...
    console.error(`❌ Error from IP: ${ip} | Form: Dubai Creek Harbour`, error);
    return NextResponse.json(
      {
        success: false,
        message:
