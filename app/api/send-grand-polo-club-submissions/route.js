...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: Grand Polo Club & Resort`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: Grand Polo Club & Resort`);
    ...
    const sanitizedData = {
      ...
      source: data.source ? escapeHtml(data.source.trim()) : "Grand Polo Club & Resort",
    };
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .data-item { margin-bottom: 15px; padding: 10px; background: white; border-left: 4px solid #b45309; }
            .label { font-weight: bold; color: #b45309; }
            .value { color: #333; }
            .footer { background: #92400e; color: white; padding: 15px; text-align: center; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="header">
            <h2>Grand Polo Club & Resort - New Inquiry</h2>
          ...
          <div class="footer">
            <p>Grand Polo Club & Resort Landing Page Submission</p>
          </div> 
        </body>
      </html>
    `;
    ...
    // Plain text version
    const emailText = `
Grand Polo Club & Resort - NEW INQUIRY
========================================
...
========================================
Grand Polo Club & Resort Landing Page
    `;
    ...
    // Send Email
    const mailOptions = {
      from: `Grand Polo Club & Resort <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      replyTo: sanitizedData.email,
      subject: `Grand Polo Club & Resort Inquiry: ${sanitizedData.fullName}`,
      text: emailText,
      html: emailHtml,
    };
    ...
    try {
      ...
      console.log(`✅ Valid submission from IP: ${ip} | Email: ${sanitizedData.email.split("@")[0]}@*** | Form: Grand Polo Club & Resort`);
    ...
    console.error(`❌ Error from IP: ${ip} | Form: Grand Polo Club & Resort`, error);
    return NextResponse.json(
      {
        success: false,
        message:
