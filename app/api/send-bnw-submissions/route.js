...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: BNW Developments`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: BNW Developments`);
    ...
    const emailHtml = `
      ...
      <html>
        ...
        <body>
          <div class="header">
            <h1>🏗️ BNW Developments</h1>
            <p>New Project Inquiry Received</p>
          </div>
          
          <div class="content">
            <div class="urgent">
              <strong>🚀 URGENT: New BNW Lead Requires Immediate Follow-up</strong>
            </div>
            ...
            <div class="details">
              ...
              <div class="field">
                ...
                <span class="value" style="color: #d97706; font-weight: bold;">${data.projectInterest || "Not specified"
      }</span>
              </div>
        ...
        "en-US",
        {
          ...
          minute: "2-digit",
            ...
            <div style="text-align: center; margin: 25px 0;">
              <a href="mailto:${data.email}?subject=BNW Follow-up: ${data.projectInterest
      }&body=Dear ${data.fullName}," 
                 style="background: #d97706; color: white; padding: 12px 30px; text-decoration: none; border-radius: 5px; display: inline-block;">
                 📧 Reply to Client
              </a>
            </div>
          </div>
          ...
          <div class="footer">
            <p>This inquiry was submitted through the BNW Developments landing page</p>
...
Call: ${data.phoneNumber}
    ...
    sendLeadEvent({
      ...
      eventSource: data.source || "BNW Developments",
    ...
    const { sheetSaved, emailSent, thankYouSent, notionSaved } = await runParallelLeadDispatch({
      sheet: appendCommunityLeadToSheet({
        source: data.source || "BNW Developments",
      ...
      leadNotification: trySendIpagMail({
        ...
        replyTo: data.email,
        subject: `🚀 New BNW Inquiry: ${data.projectInterest || data.fullName}`,
        text: emailText,
        html: emailHtml,
        fromName: "BNW Developments",
      }),
      ...
      notion: appendLeadToNotion({
        ...
        phoneNumber: data.phoneNumber,
        message: data.message || "",
        enquiryType: "BNW Developments",
        page: data.source || "BNW Developments",
        date: createdAt,
        extra: {
    ...
    console.log(
      `✅ BNW IP: ${ip} | ${data.email.split("@")[0]}@*** | sheet=${sheetSaved} email=${emailSent} thankYou=${thankYouSent} notion=${notionSaved}`
    );
    ...
    console.error(`❌ Error from IP: ${ip} | Form: BNW Developments`, error);
    return NextResponse.json(
      {
        success: false,
        message:
