...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: Rashid Yachts Marina`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: Rashid Yachts Marina`);
    ...
    const sanitizedData = {
      ...
      source: data.source ? escapeHtml(data.source.trim()) : "Rashid Yachts & Marina by EMAAR",
    };
    ...
    const emailHtml = `
      ...
      <html>
        ...
        <body>
          <div class="header">
            <h2>Rashid Yachts & Marina by EMAAR - New Inquiry</h2>
          ...
          <div class="footer">
            <p>Rashid Yachts & Marina Landing Page Submission</p>
          </div> 
        </body>
      </html>
    `;
    ...
    // Plain text version
    const emailText = `
Rashid Yachts & Marina - NEW INQUIRY
=============================
...
=============================
Rashid Yachts & Marina Landing Page
    `;
    ...
    // Send Email
    const mailOptions = {
      from: `Rashid Yachts & Marina <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      replyTo: sanitizedData.email,
      subject: `Rashid Yachts & Marina Inquiry: ${sanitizedData.fullName}`,
      text: emailText,
      html: emailHtml,
    };
    ...
    try {
      ...
      console.log(`✅ Valid submission from IP: ${ip} | Email: ${sanitizedData.email.split("@")[0]}@*** | Form: Rashid Yachts Marina`);
      ...
      console.error(`❌ Error from IP: ${ip} | Form: Rashid Yachts Marina`, emailError);
      return NextResponse.json(
        {
          success: false,
          message:
    ...
    console.error(`❌ Error from IP: ${ip} | Form: Rashid Yachts Marina`, error);
    return NextResponse.json(
      {
        success: false,
        message:
