...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: The Oasis`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: The Oasis`);
    ...
    const sanitizedData = {
      ...
      villaStyle: data.villaStyle ? escapeHtml(data.villaStyle.trim()) : "",
      message: data.message ? escapeHtml(data.message.trim()) : "",
      createdAt: data.createdAt || new Date().toISOString(),
      source: data.source ? escapeHtml(data.source.trim()) : "Palmiera 2 - The Oasis Dubai",
    };
    ...
    // HTML email template with emerald/green theme
    const emailHtml = `
      <!DOCTYPE html>
      <html>
        <head>
          ...
          <style>
            ...
            .data-item { margin-bottom: 15px; padding: 10px; background: white; border-left: 4px solid #10b981; }
            .label { font-weight: bold; color: #059669; }
            .value { color: #333; }
            .footer { background: #047857; color: white; padding: 15px; text-align: center; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="header">
            <h2>Palmiera 2 - The Oasis Dubai - New Inquiry</h2>
          ...
          <div class="content">
            ...
            ${
              sanitizedData.villaStyle
                ? `
            <div class="data-item">
              <span class="label">Villa Style:</span>
              <span class="value">${sanitizedData.villaStyle}</span>
            </div>
            `
                : ""
            }
          ...
          <div class="footer">
            <p>Palmiera 2 - The Oasis Dubai Landing Page Submission</p>
          </div> 
        </body>
      </html>
    `;
    ...
    // Plain text version
    const emailText = `
PALMIERA 2 - THE OASIS DUBAI - NEW INQUIRY
==============================
...
Palmiera 2 - The Oasis Dubai Landing Page
    ...
    // Send Email
    const mailOptions = {
      from: `Palmiera 2 - The Oasis <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      replyTo: sanitizedData.email,
      subject: `Palmiera 2 - The Oasis Inquiry: ${sanitizedData.fullName}`,
      text: emailText,
      html: emailHtml,
    };
    ...
    try {
      ...
      console.error(`❌ Error from IP: ${ip} | Form: The Oasis`, emailError);
      return NextResponse.json(
        {
          success: false,
          message:
    ...
    console.error(`❌ Error from IP: ${ip} | Form: The Oasis`, error);
    return NextResponse.json(
      {
        success: false,
        message:
