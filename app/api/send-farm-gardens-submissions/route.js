...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: Farm Garden Valley`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: Farm Garden Valley`);
    ...
    const sanitizedData = {
      ...
      villaType: data.villaType ? escapeHtml(data.villaType.trim()) : "",
      message: data.message ? escapeHtml(data.message.trim()) : "",
      createdAt: data.createdAt || new Date().toISOString(),
      source: data.source ? escapeHtml(data.source.trim()) : "Farm Gardens - The Valley",
    };
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .content { padding: 20px; background: #faf0e6; }
            .data-item { margin-bottom: 15px; padding: 10px; background: white; border-left: 4px solid #8B4513; }
            .label { font-weight: bold; color: #8B4513; }
            .value { color: #2F4F4F; }
            .footer { background: #A0522D; color: white; padding: 15px; text-align: center; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="header">
            <h2>Farm Gardens - New Inquiry</h2>
          ...
          <div class="content">
            ...
            ${
              sanitizedData.villaType
                ? `
            <div class="data-item">
              <span class="label">Villa Type:</span>
              <span class="value">${sanitizedData.villaType}</span>
            </div>
            `
                : ""
            }
          ...
          <div class="footer">
            <p>Farm Gardens Landing Page Submission</p>
          </div> 
        </body>
      </html>
    `;
    ...
    // Plain text version
    const emailText = `
FARM GARDENS - NEW INQUIRY
==========================

Name: ${sanitizedData.fullName}
Phone: ${sanitizedData.phoneNumber}
Email: ${sanitizedData.email}
${sanitizedData.villaType ? `Villa Type: ${sanitizedData.villaType}\n` : ""}${
...
Farm Gardens Landing Page
    ...
    // Send Email
    const mailOptions = {
      from: `Farm Gardens <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      replyTo: sanitizedData.email,
      subject: `Farm Gardens Inquiry: ${sanitizedData.fullName}`,
      text: emailText,
      html: emailHtml,
    };
    ...
    try {
      ...
      console.log(`✅ Valid submission from IP: ${ip} | Email: ${sanitizedData.email.split("@")[0]}@*** | Form: Farm Garden Valley`);
      ...
      console.error(`❌ Error from IP: ${ip} | Form: Farm Garden Valley`, emailError);
      return NextResponse.json(
        {
          success: false,
          message:
    ...
    console.error(`❌ Error from IP: ${ip} | Form: Farm Garden Valley`, error);
    return NextResponse.json(
      {
        success: false,
        message:
