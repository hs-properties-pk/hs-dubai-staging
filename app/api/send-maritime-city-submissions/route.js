...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: Dubai Maritime City`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: Dubai Maritime City`);
  ...
  `;
    ...
    await smtpTransporter.sendMail({
      from: `H&S Real Estate - Dubai Maritime City <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      subject: `Dubai Maritime City - New Inquiry from ${
        data.name
      }`,
      html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background-color: #1C3D5A; padding: 20px; text-align: center;">
          <h1 style="color: white; margin: 0;">Dubai Maritime City Inquiry</h1>
        </div>
        <div style="padding: 20px; background-color: #f5f5f5;">
          <p style="font-size: 16px; color: #333;">Dear Team,</p>
          <p style="font-size: 14px; color: #666;">You have received a new inquiry for Dubai Maritime City. Below are the details:</p>
          ${formTable}
    ...
    sendLeadEvent({
      ...
      eventSource: data.source || "Dubai Maritime City",
