...
export async function POST(request) {
  ...
  try {
    ...
    console.log(`🚨 Lead submitted from IP: ${ip} | User-Agent: ${userAgent.substring(0, 80)} | Form: Shahrukhz by Danube`);

    // Honeypot check - reject if bot detected (silently to avoid feedback)
    if (isBotSubmission(data)) {
      console.log(`🚫 Bot detected from IP: ${ip} | Form: Shahrukhz by Danube`);
    ...
    const sanitizedData = {
      ...
      source: data.source ? escapeHtml(data.source.trim()) : "Shahrukhz by Danube - Al Sufouh",
    };
    ...
    // HTML email template with gold theme
    const emailHtml = `
      <!DOCTYPE html>
      <html>
        <head>
          ...
          <style>
            ...
            .header { background: linear-gradient(135deg, #d4af37 0%, #f4d03f 100%); color: #000; padding: 20px; text-align: center; }
            .content { padding: 20px; background: #fffef0; }
            .data-item { margin-bottom: 15px; padding: 10px; background: white; border-left: 4px solid #d4af37; }
            .label { font-weight: bold; color: #b8941f; }
            .value { color: #333; }
            .footer { background: #b8941f; color: white; padding: 15px; text-align: center; font-size: 12px; }
          </style>
        </head>
        <body>
          <div class="header">
            <h2>Shahrukhz by Danube - New Inquiry</h2>
          ...
          <div class="footer">
            <p>Shahrukhz by Danube Landing Page Submission</p>
          </div> 
        </body>
      </html>
    `;
    ...
    // Plain text version
    const emailText = `
SHAHRUKHZ BY DANUBE - NEW INQUIRY
==============================
...
==============================
Shahrukhz by Danube Landing Page
    `;
    ...
    // Send Email
    const mailOptions = {
      from: `Shahrukhz by Danube <${process.env.EMAIL_USER}>`,
      to: process.env.EMAIL_USER,
      replyTo: sanitizedData.email,
      subject: `Shahrukhz by Danube Inquiry: ${sanitizedData.fullName}`,
      text: emailText,
      html: emailHtml,
    };
    ...
    console.error(`❌ Error from IP: ${ip} | Form: Shahrukhz by Danube`, error);
    return NextResponse.json(
      {
        success: false,
        message:
